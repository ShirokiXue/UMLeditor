...
public class BasicObject{
    ...
    public char getPosition(int X, int Y) {
        ...
        temp = CalculateDistanceQuadratic(X, Y, (r.x + (r.width / 2)), r.y);
        if (temp < min_distance) {

            min_distance = temp;
            p = 'N';
        }
        temp = CalculateDistanceQuadratic(X, Y, (r.x + (r.width / 2)), (r.y + r.height));
        if (temp < min_distance) {

            min_distance = temp;
            p = 'S';
        }
        temp = CalculateDistanceQuadratic(X, Y, (r.x + r.width), (r.y + (r.height / 2)));
        if (temp < min_distance) {

            min_distance = temp;
            p = 'E';
        }
        temp = CalculateDistanceQuadratic(X, Y, (r.x), (r.y + (r.height / 2)));
        if (temp < min_distance) {
